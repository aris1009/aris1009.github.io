...
test.describe("Dictionary Tooltips", () => {
  ...
  test("dictionary link has correct test IDs and structure", async ({
    page,
  }) => {
  ...
  test("tooltip content displays correct term and definition", async ({
    page,
  }) => {
    const dictionaryLinks = page.getByTestId(/^dictionary-link-/);
  ...
  test("multiple tooltips work independently", async ({ page }) => {
    ...
    if (linkCount >= 2) {
      ...
      const secondTooltip = page.getByTestId(
        `dictionary-tooltip-${secondTerm}`
      );
  ...
  test("tooltips have proper contrast in both light and dark modes", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("tooltip positioning works correctly", async ({ page }) => {
    ...
    if ((await dictionaryLinks.count()) > 0) {
      ...
      expect(Math.abs(linkCenter - tooltipCenter)).toBeLessThanOrEqual(
        tolerance
      );
    }
  });
  ...
  test("accessibility attributes are correct", async ({ page }) => {
    ...
    if ((await dictionaryLinks.count()) > 0) {
      ...
      // Check button role
      expect(await firstLink.evaluate((el) => el.tagName.toLowerCase())).toBe(
        "button"
      );
    }
  });

  test("tooltips appear above all content including emojis", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    if (count > 0) {
      ...
      if ((await themeToggle.count()) > 0) {
        ...
        if (darkModeTooltipBox) {
          ...
          const elementAtCenterDark = await page.evaluate(
            ({ x, y }) => {
              const element = document.elementFromPoint(x, y);
              return element ? element.closest("sl-tooltip") !== null : false;
            },
  ...
  test("dictionary tooltips work on blog posts", async ({ page }) => {
    ...
    for (const postUrl of blogPosts) {
      ...
      if (linkCount > 0) {
        ...
        // Test basic functionality - wait for Shoelace components to load
        await page.waitForFunction(
          () => window.customElements.get("sl-tooltip") !== undefined
        );
...
test.describe("Localized Dictionary Tooltips", () => {
  const testCases = [
    {
      ...
      expectedTexts: {
        ...
        phishing:
          "A cyber attack method where attackers impersonate legitimate organizations",
    ...
    },
    {
      ...
      expectedTexts: {
        encryption:
          "Η διαδικασία μετατροπής αναγνώσιμων δεδομένων σε κωδικοποιημένη μορφή",
        ...
        phishing:
          "Μια μέθοδος κυβερνοεπίθεσης όπου οι επιτιθέμενοι μιμούνται νόμιμους οργανισμούς",
    ...
    },
    {
      ...
      expectedTexts: {
        encryption:
          "Yetkisiz erişimi önlemek için okunabilir verileri kodlu forma dönüştürme işlemi",
        firewall:
          "Önceden belirlenmiş güvenlik kurallarına dayalı olarak gelen ve giden ağ trafiğini",
        malware:
          "Bilgisayar sistemlerine zarar vermek, aksatmak veya yetkisiz erişim sağlamak",
        phishing:
          "Saldırganların meşru kuruluşları taklit ederek bireyleri sahte e-postalar",
  ...
  for (const testCase of testCases) {
    test(`dictionary tooltips show ${testCase.locale} translations on ${testCase.locale} pages`, async ({
      page,
    }) => {
      ...
      if (linkCount > 0) {
        ...
        for (let i = 0; i < Math.min(linkCount, 3); i++) {
          // Test up to 3 links to avoid long tests
          ...
          const otherLocales = testCases.filter(
            (t) => t.locale !== testCase.locale
          );
          for (const otherLocale of otherLocales) {
            if (otherLocale.expectedTexts[term]) {
              expect(definitionText).not.toContain(
                otherLocale.expectedTexts[term]
              );
            }
          }
