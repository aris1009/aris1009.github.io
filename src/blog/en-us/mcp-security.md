...
> **Warning:** Existing safety mechanisms proved almost completely ineffective. Claude 3.7 Sonnet · one of the most safety-aligned models · refused malicious tool poisoning attacks less than 3% of the time. The attacks don't look malicious to the model because they use legitimate tools for unauthorized operations.
...
For sensitive operations · file writes, code commits, database changes · require human approval. Yes, this reduces automation. It also prevents a prompt injection from emptying your production database.
...
MCP adoption is accelerating. Microsoft, Google, Cloudflare, and countless startups are building on it. The convenience is real · the security risks are equally real.
